...
const template = [
  {
    title: 'Máy tính có cấu hình cao',
    prompt: 'Tư vấn về máy tính có cấu hình cao, phù hợp chơi game',
  },
  {
    title: 'Máy tính sang trọng',
    prompt: 'Những chiếc máy có ngoại hình đẹp, gọn nhẹ',
  },
  {
    title: 'Máy tính giá rẻ',
    prompt: 'Những chiếc máy tính có giá rẻ, phù hợp với sinh viên',
  },
  {
    title: 'Thế nào là chip ARM',
    prompt: 'Về dòng máy tính sử dụng chip ARM',
  },
];
...
const ChatView = () => {
  ...
  const sendMessage = async (e) => {
    ...
    try {
        const [LLMresponse, trigger] = await davinci(cleanPrompt, key, gptVersion);
        setTrigger(trigger);
  ...
  const sendMessage1 = async (e) => {
    ...
    console.log(selected);
    try {
        const promt = "Hãy tìm kiếm thông tin sản phẩm sau, lưu ý nếu có một giá trị con số được đưa ra, hãy lấy các sản phẩm bằng hoặc gần đó nhất ." + cleanPrompt + ", Lưu ý lấy tất cả các trường thông tin của các sản phẩm được đưa ra.";
        const LLMresponse = (await axios.post('http://127.0.0.1:5000/run_agent', { "prompt": promt })).data.result;
        console.log(LLMresponse);
        //const data = response.data.choices[0].message.content;
        LLMresponse && updateMessage(LLMresponse, true, aiModel);
    } catch (err) {
